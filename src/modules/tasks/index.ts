...
export const tasksModule = new Elysia({ prefix: "/tasks" }).guard({}, (app) =>
	app
		.use(guard)
		// ── CRUD ───────────────────────────────────────────────────────────────
		...
		.get(
			"/:id",
			({ user, params: { id } }) => TaskService.findById(id, user.sub),
			{
		...
		.patch(
			"/:id",
			({ user, params: { id }, body }) =>
				TaskService.update(id, user.sub, body),
			{
				params: objectIdParam,
				body: taskUpdate,
			},
		...
		.delete(
			"/:id",
			({ user, params: { id } }) => TaskService.remove(id, user.sub),
			{
				params: objectIdParam,
			},
		)
		...
		.patch(
			"/:id/status",
			({ user, params: { id }, body }) =>
				TaskService.updateStatus(id, user.sub, body.status),
		...
		.post(
			"/:id/checklist",
			({ user, params: { id }, body }) =>
				TaskService.addChecklistItem(id, user.sub, body.text),
		...
		.patch(
			...
			{
				params: t.Object({ id: objectId, itemId: objectId }),
		...
		.get(
			"/:id/subtasks",
			({ user, params: { id } }) => TaskService.listSubtasks(id, user.sub),
			{
				params: objectIdParam,
			},
		)
		.get(
			"/:id/activity",
			({ user, params: { id } }) => TaskService.listActivity(id, user.sub),
			{
				params: objectIdParam,
			},
		),
