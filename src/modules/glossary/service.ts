...
import type { GlossaryCreate, GlossaryQuery, GlossaryUpdate } from "./model";
...
export abstract class GlossaryService {
	...
	static async findById(id: string, userId: string) {
		const entry = await GlossaryEntry.findOne({
			_id: id,
			createdBy: userId,
		}).lean();
	...
	static async remove(id: string, userId: string) {
		const entry = await GlossaryEntry.findOneAndDelete({
			_id: id,
			createdBy: userId,
		}).lean();
		if (!entry) return fail(404, "Glossary entry not found");
