...
import { type AddMemberBody, Workspace, type WorkspaceCreate } from "./model";
...
export abstract class WorkspaceService {
	...
	static async addMember(
		...
		if (workspace.type !== "team")
			return fail(400, "Cannot add members to a personal workspace");
	...
	static async removeMember(
		...
		if (!workspace) return fail(404, "Workspace not found");
		if (workspace.type !== "team")
			return fail(400, "Personal workspaces have no members to remove");
		...
		if (target.role === "owner")
			return fail(400, "The workspace owner cannot be removed");
	...
	static async summary(workspaceId: string, userId: string) {
		...
		const completionRate7d =
			createdLast7 === 0
				? 0
				: Math.round((doneLast7 / createdLast7) * 1000) / 10;
		...
		return ok(
			{
				...
				last7Days: {
					...
					completionRate: completionRate7d,
				},
			},
