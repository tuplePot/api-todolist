		...
		eps: [
			{
				m: "GET",
				p: "/api/workspaces",
				d: "List workspaces the user belongs to (personal + team)",
			},
			{ m: "POST", p: "/api/workspaces", d: "Create a team workspace" },
			{
				m: "GET",
				p: "/api/workspaces/:id/summary",
				d: "Dashboard: counts per status, overdue, 7-day completion rate",
			},
			{
				m: "POST",
				p: "/api/workspaces/:id/members",
				d: "Add a member (owner/admin only)",
			},
			{
				m: "DELETE",
				p: "/api/workspaces/:id/members/:userId",
				d: "Remove a member",
			},
		],
	},
		...
		eps: [
			{
				m: "GET",
				p: "/api/tasks",
				d: "List/filter (workspace, status, priority, tag, assignedTo, dueBefore/After, search, page, limit, sortBy, order)",
			},
			{ m: "POST", p: "/api/tasks", d: "Create a task" },
			{
				m: "GET",
				p: "/api/tasks/:id",
				d: "Task detail incl. subtasks + latest activity",
			},
			{ m: "PATCH", p: "/api/tasks/:id", d: "Partial update" },
			{
				m: "DELETE",
				p: "/api/tasks/:id",
				d: "Soft delete (isArchived = true)",
			},
			{
				m: "PATCH",
				p: "/api/tasks/:id/status",
				d: "Quick status change (logs status_changed)",
			},
			{ m: "POST", p: "/api/tasks/:id/checklist", d: "Add checklist item" },
			{
				m: "PATCH",
				p: "/api/tasks/:id/checklist/:itemId",
				d: "Toggle/edit checklist item",
			},
...
const rows = groups
  ...
  <section>
    ...
    ${g.eps
			.map(
				(e) =>
					`<div class="ep"><span class="badge ${cls(e.m)}">${e.m}</span><code>${e.p}</code><span class="d">${e.d}</span></div>`,
			)
			.join("")}
  </section>`,
	)
...
export const docsModule = new Elysia().use(html()).get("/docs", () => page);
